...
void readInput(std::string filename) {
    ...
    LOGGER.x = 2; // For writing into the respective output file
    inputfile.close();
    return;
}
...
TransactionStatus tryCommit(Transaction *t) {
    ...
    TransactionStatus status = committed;

    for(auto &[key, val] : t->localSharedMemory) {
        if(t->id < shared[key]->versions.begin()->first) {
            status = aborted;
        }
            ...
            if(shared[key]->versions.size() > k) {
                shared[key]->versions.erase(shared[key]->versions.begin());
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "[k-MVTO] Avg commit delay = " << (double)totalCommitDelay.load()/totalTrans << " Avg aborts = " << (double)totalAborts.load()/totalTrans << "\n";
