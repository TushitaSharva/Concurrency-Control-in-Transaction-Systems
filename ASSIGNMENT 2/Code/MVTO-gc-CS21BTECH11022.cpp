...
std::set<int> liveTransactions;
std::atomic<int> smallestLiveTransaction{0};
...
void readInput(std::string filename) {
    ...
    LOGGER.x = 3; // For writing into the respective output file
    inputfile.close();
    return;
}
...
Transaction *begin_trans() {
    ...
    liveTransactions.insert(x);
    transactionMutex.unlock();
    return new Transaction(x);
}

void collectGarbage() { 
    for (auto item : shared) {
        item->datalock.lock();
    }

    for (auto item : shared) {
        int greatestVersionLessThanSmallestLive = item->getGreatestVersionLessThanMyId(smallestLiveTransaction); // This is bounded by transaction mutex so no worries
        for (auto it = item->versions.begin(); it != item->versions.end(); ) {
            if (it->first < greatestVersionLessThanSmallestLive) {
                it = item->versions.erase(it); // erase returns the next iterator
            } else {
...
TransactionStatus tryCommit(Transaction *t) {
    if (t->status != ongoing) {
        ...
        if (*liveTransactions.begin() != smallestLiveTransaction) {
            smallestLiveTransaction.store(*liveTransactions.begin());
            collectGarbage();
        }
        transactionMutex.unlock();
        return aborted;
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "[MVTO-gc] Avg commit delay = " << (double)totalCommitDelay.load() / totalTrans << " Avg aborts = " << (double)totalAborts.load() / totalTrans << "\n";
